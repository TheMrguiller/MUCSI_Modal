...
class CKA:
    ...
    def compare(self,
        ...
        for (x1), (x2) in tqdm(zip(dataloader1, dataloader2), desc="| Comparing features |", total=num_batches):
            ...
            _ = self.model1(image = x1['image'].to(self.device), question=x1['language_tokens'].to(self.device), padding_mask=x1['padding_mask'].to(self.device))
            _ = self.model2(image = x2['image'].to(self.device), question=x2['language_tokens'].to(self.device), padding_mask=x2['padding_mask'].to(self.device))
    ...
    def plot_results(self,
                     ...
                     title: str = None):
        fig, ax = plt.subplots()
        im = ax.imshow(self.hsic_matrix, origin='lower')
